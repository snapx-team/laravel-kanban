    ...
    public function createMembers(Request $request, $boardId)
    {
        ...
        try {
            foreach ($members as $employee) {
                ...
                Log::createLog(Auth::user()->id, Log::TYPE_KANBAN_MEMBER_CREATED, 'Added a new member <' . $member->employee->user->full_name . '> to board <' . $member->board->name . '>', null, $member->board_id, null, null, null, null);
            }
        } catch (\Exception $e) {
            return response([
                'success' => 'false',
